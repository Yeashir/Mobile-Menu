...
class ymm_Options_Framework_Interface {

	/**
	 * Generates the tabs that are used in the options menu
	 */
	static function ymm_optionsframework_tabs() {
		$counter = 0;
	 ...
	 */
	static function ymm_optionsframework_fields() {

		global $allowedtags;
		$ymm_optionsframework_settings = get_option( 'ymm_optionsframework' );

		// Gets the unique option id
		if ( isset( $ymm_optionsframework_settings['id'] ) ) {
			$option_name = $ymm_optionsframework_settings['id'];
		}
		else {
			$option_name = 'ymm_optionsframework';
		};
		...
		foreach ( $options as $value ) {
			...
			if ( has_filter( 'ymm_optionsframework_' . $value['type'] ) ) {
				$output .= apply_filters( 'ymm_optionsframework_' . $value['type'], $option_name, $value, $val );
			}
			...
			// Uploader
			case "upload":
				$output .= ymm_Options_Framework_Media_Uploader::ymm_optionsframework_uploader( $value['id'], $val, null );
			...
			case 'background':
				...
				$output .= ymm_Options_Framework_Media_Uploader::ymm_optionsframework_uploader( $value['id'], $background['image'], null, esc_attr( $option_name . '[' . $value['id'] . '][image]' ) );
				...
				$repeats = ymm_of_recognized_background_repeat();
				...
				$positions = ymm_of_recognized_background_position();
				...
				$attachments = ymm_of_recognized_background_attachment();
			...
			case "info":
				...
				if ( isset( $value['desc'] ) ) {
					$output .= apply_filters('ymm_of_sanitize_info', $value['desc'] ) . "\n";
				}
				$output .= '</div>' . "\n";
				break;
		...
		if ( ymm_Options_Framework_Interface::ymm_optionsframework_tabs() != '' ) {
			echo '</div>';
		}
	}
